"""
Test MultiLevelIndex cache flow with maintenance_pass() called synchronously
(no AsyncEngine, no background threads).

- Cache mode: L2 centroids + CacheConfig (eviction/demotion in maintenance).
- Promotion: Only SEARCH and UPDATE promote (vector + neighborhood to L0/L1).
  Insert does NOT promote; vectors land in L2 only until they are accessed via
  search or update.
- Maintenance: This test calls maintenance_pass() directly from the main thread
  twice, after search/update/erase, to see if any deadlock still occurs when
  there is no background maintenance thread.
"""
import os
import sys
...
for sub in ("", "Release", "Debug", "lib"):
    ...
    sys.path.insert(0, d)
    try:
        import _m3_async as m3  # type: ignore  # noqa: E402
        break
    except ImportError:
        sys.path.pop(0)
if m3 is None:
    try:
        ...
        print(
            "[test_multilevel_cache_sync] Skipping: C++ extension _m3_async not available. "
...
def _assert_search_contains(
    ...
    assert len(out_ids) == len(
        queries
    ), f"search returned {len(out_ids)} query results, expected {len(queries)} {msg}"
...
def main() -> None:
    ...
    print("=" * 60)
    print("MultiLevelIndex cache test (SYNC maintenance_pass, no AsyncEngine)")
    print("=" * 60)
    ...
    vecs1 = (
        np.tile(centroids[0], (n1, 1)).astype(np.float32)
        + rng.standard_normal((n1, dim)).astype(np.float32) * 0.2
    )
    ...
    vecs2_first = (
        np.tile(centroids[1], (n2, 1)).astype(np.float32)
        + rng.standard_normal((n2, dim)).astype(np.float32) * 0.15
    )
    ...
    # ---- Maintenance pass: called DIRECTLY here (no AsyncEngine) ----
    print("[11] MAINTENANCE_PASS (direct call #1): vector eviction + cluster-count demotion + cold-cluster demotion")
    idx.maintenance_pass()
    print("     -> done #1")

    _assert_search_contains(idx, vecs1[:2], [100, 101], k, nprobe, "after maintenance #1")
    print("[12] SEARCH after maintenance #1: index still consistent -> OK")
    ...
    vecs3 = (
        np.tile(centroids[2], (3, 1)).astype(np.float32)
        + rng.standard_normal((3, dim)).astype(np.float32) * 0.1
    )
    ...
    # ---- Second maintenance pass (direct) ----
    print("[17] MAINTENANCE_PASS (direct call #2)")
    idx.maintenance_pass()
    print("     -> done #2")
    ...
    print("=" * 60)
    print("All SYNC flows passed: insert (L2 only), search (promotes), update (promotes), erase, maintenance_pass x2.")
    print("=" * 60)
